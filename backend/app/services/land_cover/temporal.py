...
sawit tidak berubah jadi hutan alam. Pola yang melanggar itu hampir selalu
sisa awan/haze atau kebingungan spektral satu tahun. Aturan di bawah
memperbaikinya dengan cara KONSERVATIF: hanya tahun TENGAH (punya t-1 dan
t+1) yang bisa diubah, dan selalu ke arah kelas tetangganya -- tidak pernah
mengarang kelas baru. Tahun pertama & terakhir tidak disentuh, jadi
perubahan nyata di tahun terakhir (kebakaran 2025) tetap terlihat.

Urutan:
1. despike_symmetric      : t != t-1 dan t-1 == t+1            -> t := t-1
2. forest_gain_must_persist: t == hutan, t-1 bukan hutan/kebun,
                             t+1 != hutan                       -> t := t-1
...
3. kebun_to_forest_must_persist: t-1 == kebun, t == hutan, t+1 != hutan
                                                                -> t := kebun
   (sawit tua vs hutan spektralnya mepet; satu tahun "hutan" di tengah
   deret sawit = flicker, bukan konversi balik)
...
def forest_gain_must_persist(ee, per_year: dict[int, object], hutan_idx: int,
                             kebun_idx: int) -> dict[int, object]:
    years = sorted(per_year)
    out = dict(per_year)
    for i in range(1, len(years) - 1):
        prev, cur, nxt = per_year[years[i - 1]], per_year[years[i]], per_year[years[i + 1]]
        gain = cur.eq(hutan_idx).And(prev.neq(hutan_idx)).And(prev.neq(kebun_idx))
        transient = gain.And(nxt.neq(hutan_idx))
        out[years[i]] = cur.where(transient, prev).rename("class_idx")
    return out


def kebun_to_forest_must_persist(ee, per_year: dict[int, object], hutan_idx: int,
                                 kebun_idx: int) -> dict[int, object]:
    years = sorted(per_year)
    out = dict(per_year)
    for i in range(1, len(years) - 1):
        prev, cur, nxt = per_year[years[i - 1]], per_year[years[i]], per_year[years[i + 1]]
        flicker = prev.eq(kebun_idx).And(cur.eq(hutan_idx)).And(nxt.neq(hutan_idx))
...
def apply_transition_rules(ee, per_year: dict[int, object], class_idx_of: dict[str, int]
    ...
    hutan, kebun = class_idx_of["hutan"], class_idx_of["kebun"]
    out = despike_symmetric(ee, per_year)
    out = forest_gain_must_persist(ee, out, hutan, kebun)
    out = kebun_to_forest_must_persist(ee, out, hutan, kebun)
